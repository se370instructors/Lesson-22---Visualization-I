...
-   `geom_line()`: adds lines connecting data
...
1.  Make a scatterplot of `mpg` vs. `disp`. Change the color of the points to red.
    ...
    ggplot(data = mtcars, aes(x = mpg, y = disp)) + geom_point(color = "red")
    ```
...
2.  Add a line to your scatterplot.
    ...
    ggplot(data = mtcars, aes(x = mpg, y = disp)) + geom_point(color = "red") + geom_line()
    ```
...
3.  Add `color = factor(cyl)` within your `aes()` argument. What happens?
    ...
    ggplot(data = mtcars, aes(x = mpg, y = disp, color = factor(cyl))) + geom_point()
    ```

4.  Add the minimal theme to your plot.

    ```{r theme}
    ggplot(data = mtcars, aes(x = mpg, y = disp, color = factor(cyl))) + geom_point() + theme_minimal()
    ```

5.  Add a title using `ggtitle()` - and then center it!

    ```{r title and center it}
    ggplot(data = mtcars, aes(x = mpg, y = disp, color = factor(cyl))) + geom_point() + theme_minimal() + ggtitle("MPG v. DISP") + theme(plot.title = element_text(hjust = 0.5))
    ```
...
7.  Use `facet_wrap()` with the `cyl` variable to create 3 versions of your bar plot from the above question - one for each value of `cyl`.

    ```{r facet_grid}
    ggplot(data = mtcars, aes(x = gear)) + geom_bar() + facet_wrap(~cyl)
    ```
